    ...
    for day in range(100):
        ...
        for i, row in enumerate(floor[down:up+1], down):
            for j, tile in enumerate(row[left:right+1], left):
                black_tiles = (floor[i-1, j:j+2].sum()
                               + floor[i, j-1] + floor[i, j+1]
                               + floor[i+1, j-1:j+1].sum())
...
plt.figure()
plt.scatter(white_tiles[:, 0], white_tiles[:, 1], s=20, c='w', edgecolors='k',
            marker='h', linewidth=0.1)
plt.scatter(black_tiles[:, 0], black_tiles[:, 1], s=20, c='k', marker='h')
